...
export class PlayedGameService{
  ...
  getGame(playedGameId: string): Observable<PlayedGame>{
    return this.http.get<PlayedGame>(`${environment.api}/playedgames/${playedGameId}`);
  }

  getAllGames(): Observable<PlayedGameList>{
    return this.http.get<PlayedGameList>(`${environment.api}/playedgames`);
  }

  initializeGame(baseGameId: number): Observable<PlayedGame>{
    return this.http.put<PlayedGame>(`${environment.api}/playedgames/${baseGameId}`, null);
  }

  startGame(playedGameId: string): Observable<PlayedGame>{
    return this.http.put<PlayedGame>(`${environment.api}/playedgames/${playedGameId}/start`, null);
  }

  deleteGame(playedGameId: string): Observable<string>{
    return this.http.delete<string>(`${environment.api}/playedgames/${playedGameId}`);
  }

  // ROUND ------------------------------------------------------------------

  getActiveRound(playedGameId: string): Observable<Round>{
    return this.http.get<Round>(`${environment.api}/playedgames/${playedGameId}/round`);
  }

  setNextRound(playedGameId: string, playerLogin: string): Observable<Round>{
    return this.http.put<Round>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/round/next`, null);
  }
  ...
  selectRoundOpiton(playedGameId: string, playerLogin: string, fieldOption: FieldOptionEnum){
    return this.http.put(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/action/${fieldOption}`, null);
  }

  // BOARD + FIELDS ----------------------------------------------------------

  getBoard(playedGameId: string): Observable<PlayedBoard>{
    return this.http.get<PlayedBoard>(`${environment.api}/playedgames/${playedGameId}/board`);
  }

  getFields(playedGameId: string): Observable<FieldList>{
    return this.http.get<FieldList>(`${environment.api}/playedgames/${playedGameId}/board/fields`);
  }

  getField(playedGameId: string, fieldId: number): Observable<Field>{
    return this.http.get<Field>(`${environment.api}/playedgames/${playedGameId}/board/fields/${fieldId}`);
  }

  getGameBossField(playedGameId: string): Observable<number>{
    return this.http.get<number>(`${environment.api}/playedgames/${playedGameId}/board/fields/boss`);
  }

  getGameBridgeField(playedGameId: string) :Observable<number>{
    return this.http.get<number>(`${environment.api}/playedgames/${playedGameId}/board/fields/bridge`);
  }

  // CARDS----------------------------------------------------------

  getCardsDeck(playedGameId: string): Observable<CardList>{
    return this.http.get<CardList>(`${environment.api}/playedgames/${playedGameId}/cards/deck`);
  }

  getUsedCardsDeck(playedGameId: string): Observable<CardList>{
    return this.http.get<CardList>(`${environment.api}/playedgames/${playedGameId}/cards/used`);
  }

  getCardFromDeck(playedGameId: string, cardId: number): Observable<Card>{
    return this.http.get<Card>(`${environment.api}/playedgames/${playedGameId}/cards/deck/${cardId}`);
  }

  getCardFromUsedCardDeck(playedGameId: string, cardId: number): Observable<Card>{
    return this.http.get<Card>(`${environment.api}/playedgames/${playedGameId}/cards/used/${cardId}`);
  }

  moveCardFromCardDeckToUsedCardDeck(playedGameId: string, cardId: number): Observable<PlayedGame>{
    return this.http.put<PlayedGame>(`${environment.api}/playedgames/${playedGameId}/cards/used/${cardId}`, null);
  }

  moveCardFromPlayerHandToUsedCardDeck(playedGameId: string, playerLogin: string, cardId: number): Observable<PlayedGame>{
    return this.http.put<PlayedGame>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/cards/used/${cardId}`, null);
  }

  moveCardFromPlayerToPlayer(playedGameId: string, playerFromLogin: string, cardId: number, playerToLogin: string){
    return this.http.put(`${environment.api}/playedgames/${playedGameId}/players/${playerFromLogin}/cards/${cardId}/players/${playerToLogin}`, null)
  }

  moveItemCardFromDeckToPlayerHand(playedGameId: string, cardId: number, playerLogin: string): Observable<PlayedGame>{
    return this.http.put<PlayedGame>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/cards/hand/${cardId}`, null);
  }

  moveCardToPlayerTrophies(playedGameId: string, playerLogin: string, cardId: number): Observable<PlayedGame>{
    return this.http.put<PlayedGame>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/cards/trophies/${cardId}`, null);
  }

  drawRandomCard(playedGameId: string, playerLogin: string): Observable<Card>{
    return this.http.get<Card>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/cards/deck/draw`);
  }

  // CHARACTERS -------------------------------------------------------

  getAllCharacters(playedGameId: string): Observable<CharacterList>{
    return this.http.get<CharacterList>(`${environment.api}/playedgames/${playedGameId}/characters`);
  }

  getCharacter(playedGameId: string, characterId: number): Observable<PlayedGameCharacter>{
    return this.http.get<PlayedGameCharacter>(`${environment.api}/playedgames/${playedGameId}/characters/${characterId}`);
  }

  getCharactersInUse(playedGameId: string): Observable<CharacterList>{
    return this.http.get<CharacterList>(`${environment.api}/playedgames/${playedGameId}/characters/used`);
  }

  getCharacterNotInUse(playedGameId: string): Observable<CharacterList>{
    return this.http.get<CharacterList>(`${environment.api}/playedgames/${playedGameId}/characters/free`);
  }

  // PLAYERS -------------------------------------------------------------

  getPlayers(playedGameId: string): Observable<PlayerList>{
    return this.http.get<PlayerList>(`${environment.api}/playedgames/${playedGameId}/players`);
  }

  getPlayer(playedGameId: string, playerLogin: string): Observable<Player>{
    return this.http.get<Player>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}`);
  }

  getPlayersOnField(playedGameId: string, fieldId: number): Observable<PlayerList>{
    return this.http.get<PlayerList>(`${environment.api}/playedgames/${playedGameId}/players/field/${fieldId}`);
  }

  getPlayersCharacter(playedGameId: string, playerLogin: string): Observable<PlayedGameCharacter>{
    return this.http.get<PlayedGameCharacter>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/character`);
  }

  getCardsFromPlayerHand(playedGameId: string, playerLogin: string): Observable<ItemCardList>{
    return this.http.get<ItemCardList>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/cards/hand`);
  }

  getHealthCardsFromPlayerHand(playedGameId: string, playerLogin: string): Observable<ItemCardList>{
    return this.http.get<ItemCardList>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/cards/hand/health`);
  }

  getStrengthCardsFromPlayerHand(playedGameId: string, playerLogin: string): Observable<ItemCardList>{
    return this.http.get<ItemCardList>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/cards/hand/strength`);
  }

  getCardFromPlayerHand(playedGameId: string, playerLogin: string, cardId: number): Observable<ItemCard>{
    return this.http.get<ItemCard>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/cards/hand/${cardId}`);
  }

  getTrophies(playedGameId: string, playerLogin: string): Observable<EnemyCardList>{
    return this.http.get<EnemyCardList>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/cards/trophies`);
  }

  addPlayerToGameByLogin(playedGameId: string, playerLogin: string): Observable<PlayedGame>{
    return this.http.put<PlayedGame>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}`, null);
  }

  selectCharacter(playedGameId: string, playerLogin: string, characterId: number): Observable<PlayedGame>{
    return this.http.put<PlayedGame>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/character/${characterId}`, null);
  }

  checkPossibleNewPositions(playedGameId: string, playerLogin: string): Observable<FieldList>{
    return this.http.get<FieldList>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/field/move/fields`);
  }

  changeFieldPositionOfCharacter(playedGameId: string, playerLogin: string, fieldId: number): Observable<PlayedGame>{
    return this.http.put<PlayedGame>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/field/${fieldId}`, null);
  }

  getPlayersPossibleActions(playedGameId: string, playerLogin: string): Observable<FieldOptionList>{
    return this.http.get<FieldOptionList>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/field/options`);
  }

  getPlayersToFightWith(playedGameId: string, playerLogin: string): Observable<PlayerList>{
    return this.http.get<PlayerList>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/field/options/players`);
  }

  getEnemiesToFightWith(playedGameId: string, playerLogin: string): Observable<EnemyCardList>{
    return this.http.get<EnemyCardList>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/field/options/enemies`);
  }

  blockTurnsOfPlayerByNumber(playedGameId: string, playerLogin: string, blockedNum: number): Observable<Player>{
    return this.http.put<Player>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/block/${blockedNum}`, null);
  }

  automaticallyBlockTurnsOfPlayer(playedGameId: string, playerLogin: string): Observable<Player>{
    return this.http.put<Player>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/block`, null);
  }

  // ACTIONS

  rollDice(playedGameId: string, playerLogin: string): Observable<number>{
    return this.http.get<number>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/roll`);
  }

  handleFightWithEnemyCard(playedGameId: string, playerLogin: string, enemyCardId: number): Observable<FightResult>{
    return this.http.put<FightResult>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/fight/enemy/${enemyCardId}`, null);
  }
  ...
  handleFightWithEnemyField(playedGameId: string, playerLogin: string, playerRoll: number, enemyRoll: number): Observable<FightResult>{
    return this.http.put<FightResult>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/roll/${playerRoll}/enemy/roll/${enemyRoll}`, null);
  }*/

  handleFightWithPlayer(playedGameId: string, playerLogin: string, enemyPlayerLogin: string): Observable<FightResult>{
    return this.http.put<FightResult>(`${environment.api}/playedgames/${playedGameId}/players/${playerLogin}/fight/player/${enemyPlayerLogin}`, null);
  }
